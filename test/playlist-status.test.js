...
test('주간 초기화 뒤에도 주차별 곡과 신청자 이름을 보관한다', () => {
  ...
  try {
    database.addSong('guild-a', {
      day: '월', videoId: 'song-a', title: '노래', artist: '미쿠', url: 'https://youtu.be/song-a',
      userId: 'user-a', userName: '홍길동',
    });
    database.archiveWeek('guild-a', '2026-07-12');
    database.resetWeekly('guild-a');

    assert.deepEqual(database.historyWeeks('guild-a'), ['2026-07-12']);
    assert.deepEqual(database.historyDaySongs('guild-a', '2026-07-12', '월').map(({ title, user_name: name }) => ({ title, name })), [
      { title: '노래', name: '홍길동' },
    ]);
...
test('저장된 요일 메시지 조회가 일시적으로 실패하면 새 메시지를 만들지 않는다', async () => {
  const { directory, database } = openDatabase('wekkly-fetch-failure-');
  try {
    database.setGuildChannels('guild-a', 'request-a', 'announcement-a');
    const messageIds = { 월: 'm', 화: 't', 수: 'w', 목: 'h', 금: 'f' };
    database.setGuildDayMessages('guild-a', messageIds, '2026-07-12');
    let sends = 0;
    const error = new Error('temporary');
    error.code = 500;
    const channel = {
      isTextBased: () => true,
      messages: { fetch: async () => { throw error; } },
      send: async () => { sends += 1; },
    };

    await assert.rejects(() => ensureWeeklyStatus({
      client: { channels: { fetch: async () => channel } }, database, guildId: 'guild-a',
      key: '2026-07-12', forceEdit: true, day: '월',
    }), /temporary/);

    assert.equal(sends, 0);
    assert.deepEqual(JSON.parse(database.guildChannels('guild-a').day_message_ids), messageIds);
  } finally {
    database.close();
    rmSync(directory, { recursive: true, force: true });
  }
...
test('지난 플리는 주차에서 요일과 곡을 차례로 비공개 조회한다', async () => {
  ...
  try {
    ...
    await handler.history({
      guildId: 'guild-a', options: { getString: () => '2026-07-12' },
      reply: async (payload) => replies.push(payload),
    });
    await handler.historyDay({
      guildId: 'guild-a', customId: 'history:day:2026-07-12:화',
      update: async (payload) => dayUpdates.push(payload),
    });
    await handler.historySong({
      guildId: 'guild-a', customId: 'history:song:2026-07-12:화', values: ['song-a'],
...
test('주차가 바뀌면 기존 곡을 보관하고 다섯 요일 메시지를 비운다', async () => {
  ...
  try {
    database.setGuildChannels('guild-a', 'request-a', 'announcement-a');
    database.setGuildDayMessages('guild-a', { 월: 'm', 화: 't', 수: 'w', 목: 'h', 금: 'f' }, '2026-07-12');
    database.setMeta('guild-a', 'last_weekly_reset', '2026-07-12');
    database.addSong('guild-a', {
      day: '월', videoId: 'song-a', title: '노래', artist: '미쿠', url: 'https://youtu.be/song-a',
      userId: 'user-a', userName: '홍길동',
    });
    ...
    assert.equal(database.daySongs('guild-a', '월').length, 0);
    assert.equal(database.historyDaySongs('guild-a', '2026-07-12', '월').length, 1);
...
test('관리자 초기화는 채널 설정을 유지하고 다섯 요일 메시지를 비운다', async () => {
  const { directory, database } = openDatabase('wekkly-manual-reset-');
  try {
    database.setGuildChannels('guild-a', 'request-a', 'announcement-a');
    database.setGuildDayMessages('guild-a', { 월: 'm', 화: 't', 수: 'w', 목: 'h', 금: 'f' }, '2026-07-12');
    database.addSong('guild-a', {
    ...
    await handler.reset({
      ...
      options: { getString: () => '초기화' },
      client: { channels: { fetch: async () => channel } },
      reply: async () => {},
    });

    assert.equal(database.daySongs('guild-a', '월').length, 0);
    assert.equal(database.guildChannels('guild-a').request_channel_id, 'request-a');
    assert.equal(edits.length, 5);
  } finally {
    database.close();
    rmSync(directory, { recursive: true, force: true });
