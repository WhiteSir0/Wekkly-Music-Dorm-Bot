...
test('Discord 명령어 JSON이 모두 생성된다', () => {
  ...
  assert.deepEqual(commands.map(({ name }) => name), ['도움말', '정보', '신청', '보기', '지난플리', '채널설정', '플리제한', '셔플', '삭제', 'db초기화']);
  ...
  assert.equal(commands.find(({ name }) => name === '채널설정').options.find(({ name }) => name === '신고채널').required, true);
  assert.equal(commands.find(({ name }) => name === '보기').options[0].required, true);
  assert.equal(commands.find(({ name }) => name === '지난플리').options.find(({ name }) => name === '주차').autocomplete, true);
});
...
test('길드별 채널 설정을 SQLite에 영구 저장한다', () => {
  ...
  try {
    ...
    assert.deepEqual({ ...database.guildChannels('guild-a') }, {
      ...
      weekly_message_id: null, weekly_message_key: null, day_message_ids: '{}', report_channel_id: 'announcement-a',
    });
...
test('길드 안내 메시지 ID를 안전하게 마이그레이션하고 영구 저장한다', () => {
  ...
  try {
    ...
    assert.equal(database.guildChannels('guild-a').report_channel_id, null);
  } finally {
    database?.close();
    rmSync(directory, { recursive: true, force: true });
  }
...
test('채널 설정은 저장된 안내 메시지를 새로 만들지 않고 교체한다', async () => {
  ...
  const dayIds = Object.fromEntries(['월', '화', '수', '목', '금'].map((day) => [day, `${day}-message`]));
  const requestChannel = {
    id: 'request-a', toString: () => '<#request-a>',
    isTextBased: () => true,
    messages: { fetch: async (id) => id === 'guide-a' ? guideMessage : { edit: async (payload) => weeklyEdits.push({ id, payload }) } },
  ...
  const reportChannel = { id: 'report-a', toString: () => '<#report-a>' };
  const handler = new CommandHandler({
    database: {
      guildChannels: () => ({
        request_channel_id: 'request-a', announcement_channel_id: 'announcement-a', guide_message_id: 'guide-a',
        weekly_message_id: null, weekly_message_key: '2026-07-12', day_message_ids: JSON.stringify(dayIds),
      }),
      setGuildChannels: () => {},
...
test('스케줄러는 길드별로 설정된 공지 채널을 사용한다', async () => {
  ...
  const database = {
    ...
    guildChannels: (guildId) => ({
      ...
      weekly_message_id: null, weekly_message_key: '2026-07-12',
      day_message_ids: JSON.stringify({ 월: 'm', 화: 't', 수: 'w', 목: 'h', 금: 'f' }),
    }),
  };
  const scheduler = new Scheduler({
...
test('봇이 늦게 시작해도 이번 주 현황 메시지를 한 번만 보낸다', async () => {
  ...
  try {
    database = new MusicDatabase(join(directory, 'music.db'));
    database.setGuildChannels('guild-a', 'request-a', 'announcement-a');
    database.setGuildWeeklyMessage('guild-a', 'old-weekly', '2026-07-12');
    const sends = [];
    let legacyDeletes = 0;
    const channel = {
      isTextBased: () => true,
      messages: { fetch: async (id) => id === 'old-weekly' ? { delete: async () => { legacyDeletes += 1; } } : null },
      send: async (payload) => {
        sends.push(payload);
        return { id: 'weekly-a' };
      },
    ...
    assert.equal(legacyDeletes, 1);
    assert.deepEqual(sends.map(({ files }) => files[0].name), ['월-playlist.png', '화-playlist.png', '수-playlist.png', '목-playlist.png', '금-playlist.png']);
    assert.deepEqual({ ...database.guildChannels('guild-a') }, {
      guild_id: 'guild-a', request_channel_id: 'request-a', announcement_channel_id: 'announcement-a',
      guide_message_id: null, weekly_message_id: null, weekly_message_key: '2026-07-12',
      day_message_ids: JSON.stringify({ 월: 'weekly-a', 화: 'weekly-a', 수: 'weekly-a', 목: 'weekly-a', 금: 'weekly-a' }),
      report_channel_id: 'announcement-a',
    });
  } finally {
    database?.close();
    rmSync(directory, { recursive: true, force: true });
