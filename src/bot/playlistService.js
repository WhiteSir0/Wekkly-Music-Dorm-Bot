...
export class PlaylistService {
  ...
  validate(userId, day) {
    ...
    if (this.database.dayCount(day) >= MAX_SONGS[day]) return { ok: false, message: '해당 요일 플레이리스트가 가득 찼습니다.' };
    if (!exclusive && this.database.userCount(userId) >= MAX_WEEKLY_SONGS) return { ok: false, message: '주간 신청 가능 횟수(2곡)를 모두 사용했습니다.' };
    return { ok: true };
  }
