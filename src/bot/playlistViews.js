...
export async function handleView(database, interaction) {
  await interaction.reply({ ...dayStatusPayload(database, interaction.guildId, interaction.options.getString('요일')), flags: MessageFlags.Ephemeral });
}

export async function handleHistory(database, interaction) {
  const key = interaction.options.getString('주차');
  if (!database.historyWeeks(interaction.guildId).includes(key)) {
...
export async function handleHistoryAutocomplete(database, interaction) {
  ...
  const choices = database.historyWeeks(interaction.guildId).map((key) => ({ name: weekLabel(key), value: key }))
...
export async function handleHistoryDay(database, interaction) {
  ...
  await interaction.update({ ...historyDayStatusPayload(database, interaction.guildId, key, day), attachments: [] });
}

export async function handleHistorySong(database, interaction) {
  const [, , key] = interaction.customId.split(':');
  const song = database.historySong(interaction.guildId, key, interaction.values[0]);
  if (!song) {
